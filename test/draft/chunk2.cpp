...
struct object_pool
{
    ...
    {
        chunk item; // NOLINT
        constexpr link_item() noexcept : item(this) {}
    };
    ...
    struct chunk
    {
        ...
        constexpr explicit chunk(link_item *parent_ptr) noexcept
        ...
        {
            return std::popcount<status_type>(status_);
        }
      ...
      private:
        link_item *parent_; // 添加父对象指针
        status_type status_;
        std::array<object_storage_type, block_count> buffer_;

        constexpr void set_one(std::uint8_t index) noexcept // NOLINT
        {
            // NOTE: status_type{1} 是必须的。否则
    ...
    constexpr explicit object_pool(std::size_t init_count)
    {
  ...
  private:
    ...
    std::array<intrusive_list<link_item>, pool_size> pool_;
};

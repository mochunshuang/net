...
{
    return [=](span_param_in sp) constexpr noexcept -> parse_result {
        ...
        (
            [&]() constexpr noexcept -> void {
                ...
                total_consumed += res.value();
                current_sp = current_sp.subspan(total_consumed);
            }(),
            ...);
        ...
        (
            [&] constexpr noexcept {
                ...
                if (res)
                {
                    ...
                    final_result.consumed = res.value();
                }
            }(),
            ...);
...
{
    {
        ...
        static_assert(CHAR('a')(std::array<OCTET, 2>{'a', 'b'}).value() == 1);

        // 测试 CHAR 失败
        static_assert(!CHAR('a')(std::array<OCTET, 1>{'b'}));
        static_assert(!CHAR('a')(std::array<OCTET, 0>{})); // 空输入
    }

    {
        // 成功序列
        constexpr auto seq_ab = SEQUENCE(CHAR('a'), CHAR('b'));
        assert(seq_ab(std::array<OCTET, 2>{'a', 'b'}).consumed == 2);
    ...
    // 测试混合组合：a* [b] c+
    {
        ...
        assert(complex_rule(std::array<OCTET, 2>{'b', 'c'}).success);   // 没有 a

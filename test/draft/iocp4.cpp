...
struct iocp_context
{
  ...
  public:
    ...
    void post_write(SOCKET socket, const char *data, size_t len, bool closeAfter = true)
    {
        auto op = new IocpWriteOp(socket, data, len, closeAfter);
        DWORD sent = 0;
        ...
        while (true)
        {
            ...
            case IO_READ: {
                ...
                thread_pool.submit([this, read_op, bytes] {
                    std::string request_data(read_op->buffer, bytes);
                    std::string path;
                    ...
                    post_write(read_op->socket, response.data(), response.size(), true);
                    delete read_op;
                });
                break;
            }

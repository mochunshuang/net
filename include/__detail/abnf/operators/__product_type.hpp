...
namespace mcs::abnf::operators
{
    ...
    constexpr bool // NOLINTNEXTLINE
        is_product_type_v<product_type<U...>> = true;
...
{
    template <typename... T>
    struct tuple_size<::mcs::abnf::operators::product_type<T...>> // NOLINT
        : std::integral_constant<std::size_t, sizeof...(T)>
    {
    };

    template <std::size_t I, typename... T>
    struct tuple_element<I, ::mcs::abnf::operators::product_type<T...>> // NOLINT
    {
        using type = std::decay_t<
            decltype(std::declval<::mcs::abnf::operators::product_type<T...>>()
                         .template get<I>())>;
    };
}; // namespace std
...
namespace std
{
    // for std::get<>
    template <std::size_t I, typename T>
        requires ::mcs::abnf::operators::is_product_type<std::remove_cvref_t<T>>

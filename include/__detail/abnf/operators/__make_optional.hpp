...
namespace mcs::abnf::operators
{
    ...
    struct make_optional : product_type<Rule>
    {
        ...
        static constexpr std::string build(const result_type &ctx) noexcept
        {
    ...
    template <typename T>
    make_optional(T &&r) -> make_optional<std::decay_t<T>>;

    template <typename T> // NOLINTNEXTLINE
    inline constexpr bool is_operators_rule<make_optional<T>> = true;
